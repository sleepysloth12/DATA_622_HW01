...
The random forest classifier aimed to predict the presence of diabetes using various health and demographic features in the dataset. After ensuring the target variable was treated as a categorical factor, the dataset was split into training and testing subsets, with 70% allocated to training. A simplified grid search was performed to identify the optimal number of features (`mtry`) to consider at each split, helping to balance model performance with runtime. The final model was trained using 300 trees with the best `mtry` value, yielding reliable and interpretable predictions. Evaluation on the test set showed strong classification performance, with a detailed confusion matrix highlighting accuracy, precision, recall, and F1-score. Additionally, the variable importance plot revealed the top contributing features, helping to understand which factors most strongly correlate with diabetes presence. This model offers a robust tool for diabetes prediction, leveraging key health indicators for accurate classification.

```{r}
# Load necessary libraries
if (!require(randomForest)) install.packages("randomForest")
library(randomForest)
if (!require(caret)) install.packages("caret") # For additional performance metrics
library(caret)

# Ensure the response variable (diabetes) is treated as a factor
...
# Simplified Hyperparameter Tuning with Manual Grid
# We'll use a small grid for mtry and limit trees for faster results
mtry_grid <- c(2, 4, 6)
best_mtry <- mtry_grid[1] # Default initial mtry
best_oob_error <- Inf

for (m in mtry_grid) {
  # Train model with fewer trees during tuning
  ...
  # Calculate Out-of-Bag error and update best_mtry if improvement
...
rf_model <- randomForest(
  diabetes ~ ., 
  data = train_data, 
  ntree = 300,    # A moderate number of trees to balance performance and runtime
  mtry = best_mtry, 
  importance = TRUE
)
