...
```
The Random Forest model produced very week results, with a high MSE on the housing price dataset even though we used only the numerical columns, it probably the data is from known source and collected carefully also it very small data set.
...
# The Neutral network model produced very weak results, with a high MAE on the housing price dataset even though we used only the numerical columns, it probably because the data was not an image data.
